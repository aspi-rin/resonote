...
describe("global context editor", () => {
  ...
  it("saves with the expected revision, keeping existing ids and leaving new ones empty", async () => {
    scriptAppDefaults({
      globalContext: globalContextDocument({
        content: globalContent({
          recurringPeople: [{ aliases: ["小张"], canonicalName: "张三", commonAsrErrors: [], description: "", id: "person-1" }],
    ...
    expect(content.recurringPeople[0].aliases).toEqual(["小张"]);
  ...
  it("reorders and removes entity rows without touching the remaining ids", async () => {
    scriptAppDefaults({
      globalContext: globalContextDocument({
        content: globalContent({
          recurringPeople: [
            { aliases: [], canonicalName: "张三", commonAsrErrors: [], description: "", id: "person-1" },
    ...
    expect(names()).toEqual(["Alex Chen", "张三"]);
