...
export function TextField({ label, value, onChange, type = "text", disabled = false, className = "", placeholder = "", readonly = false }: { label: string; value: string; onChange: (value: string) => void; type?: "text" | "url"; disabled?: boolean; className?: string; placeholder?: string; readonly?: boolean }) { return <label class={`field ${className}`}><span>{label}</span><input disabled={disabled} placeholder={placeholder} readOnly={readonly} type={type} value={value} onInput={(event) => onChange(event.currentTarget.value)} /></label>; }
